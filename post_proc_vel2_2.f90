...
real(rprec)                       ::               dx=L_x/nx,dy=L_y/ny,dz=z_i/(nz_tot)
  real(rprec),dimension(:),allocatable        ::weight_x,weight_y,weight_z,d50,tot
...
!
!
!  write(file,'(A,I2.2,A)')'d50_av_z_incl', int((384-jz-1)*100/385),'.dat'
!  open(34,FILE=file,status='unknown',position='append')
!  write(34,*) 'x,d50'
!  do jx = 48,5*nx/6
!    t_vol = sum(tot_vol(jx,jy,jz,1:npcon))
!    tot = 0._rprec
...
!     do ip=1,npcon
!      
!       dsd(counter,ip) =(pcon(59,yps,271,ip))!&
...
!
   ...
   do jx=jx_start,jx_end,5
     do ip=1,npcon

       dsd(counter,ip) = sum(pcon(jx,jy_start:jy_end,jz_start:jz_end,ip))/&
                         size(pcon(jx,jy_start:jy_end,jz_start:jz_end,ip))
     enddo

   counter=counter+1
   enddo
!
   write(file,'(A,A,I3.3,A)')TRIM(path), "dsd_z_tot_calc", int((jz_end-5)*100/384),".dat"
!   write(file,'(A,A,I3.3,A)')TRIM(path), "dsd_z_", int((jz-1)*100._rprec/nz_tot),".dat"
!
     open(20,FILE=file)
   do counter=1,count_n
     write(20,505) (jx_start+(counter-1)*5)*L_x/nx,sum(pi/6._rprec*dsd(counter,:)*diameter**3),&
                  (dsd(counter,ip)/d_d(ip),ip=1,npcon)
   enddo
close(20)
!
