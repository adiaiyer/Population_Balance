...
!!!xxxxxxxxxx----------VIJ----------XXXXXXXXXXXXXXXXXXXXX
!DY Define ubar and thetabar in the module instead of in subroutine ic_scal
!DY Use them in the main subroutine when applying fringe zone in outflow
!DY After being calculated as IC, ubar and thetabar are saved in restart file in
!io mudule
...
implicit none
!DY Move the definition of the initial velocity and temperature profiles to the
!module, which can be used in the main program with the fringe zone
...
write(*,*) "ubar,vbar",maxval(ubar),maxval(vbar)
...
call mpi_gatherv (ubar(1), size(ubar(1:nz)), MPI_RPREC,&
     ubar_tot, sendcounts, displs,       &
     MPI_RPREC, rank_of_coord(0), comm, ierr)
call mpi_gatherv (vbar(1), size(ubar(1:nz)), MPI_RPREC,&
     vbar_tot(1), sendcounts, displs,       &
     MPI_RPREC, rank_of_coord(0), comm, ierr)
call mpi_gatherv (wbar(1), size(ubar(1:nz)), MPI_RPREC,&
     wbar_tot(1), sendcounts, displs,       &
     MPI_RPREC, rank_of_coord(0), comm, ierr)
call mpi_gatherv (thetabar(1), size(ubar(1:nz)), MPI_RPREC,&
