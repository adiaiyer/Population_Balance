...
!!$!DY For debugging
!!$!DY Start here
!!$real(kind=rprec), dimension(ld,ny) :: F_LM0
!!$!DY End here
!!$!DY*****************
...
if (inilag) then
  if ((.not. F_LM_MM_init) .and. (jt == cs_count .or. jt == DYN_init)) then
     ...
     !DY********************
     !DY For debugging
     !DY Set F_LM = 0 initially, so that Cs2=0
     !DY Start here
     !DY F_LM (:,:,jz) = 0.03_rprec*MM
     F_LM (:,:,jz) = 0._rprec
     !DY End here
     !DY********************
    ...
    if(inflow)then
       ...
       NN=Tn
       QN(jx_s-x_relax+1:jx_s,1:ny)=0._rprec
       F_QN(jx_s-x_relax+1:jx_s,1:ny,jz)=0._rprec
    endif
...
!!$F_LM(:,:,jz)= max(real(-1E-2),real(F_LM(:,:,jz)))
!!$if(coord.eq.nproc-1.and.jz.eq.nz/2) then
!!$   do jy=1,10
!!$      do jx=1,10
!!$         if(F_LM(jx,jy,jz).lt.0.) then
!!$            print*, 'jx,jy,F_LM',jx,jy,F_LM(jx,jy,jz)
!!$         endif
!!$      enddo
!!$   enddo
!!$endif
...
!!$do jy=1,ny
!!$   do jx=1,nx
!!$      if(LM(jx,jy).lt.0.) then
!!$         write(200+coord,201) jx,jy,jz,LM(jx,jy),F_LM0(jx,jy),F_LM(jx,jy,jz),epsi(jx,jy),Tn(jx,jy)
!!$      endif
!!$   enddo
!!$enddo
!!$201 format(3i5,6e12.4)
!!$!DY Start here
!!$!DY End here
!!$!DY***********************
...
if (inilag) then
  if ((.not. F_QN_NN_init) .and. (jt == cs_count .or. jt == DYN_init)) then
    ...
    !DY********************
    !DY For debugging
    !DY Set F_QN = 0 initially, so that Cs2=0
    !DY Start here
...
!!$F_QN(:,:,jz)= max(real(-1E-2),real(F_QN(:,:,jz)))
!DY End here
!DY***********************
!!$Tn_QN(:,:,jz)=Tn
...
! This basically saves the Cs calculated using the scale-invariance assumption
! within the scale-dependent model to Cs_Ssim
Cs_Ssim=Cs_opt2_2d
end if

!print *,'jt,jz,beta,betaclip,Cs_opt2',jt,jz,sum(Beta(1:nx,1:ny,jz))/(nx*ny),&
!sum(Betaclip(1:nx,1:ny,jz))/(nx*ny),sum(Cs_opt2(1:nx,1:ny,jz))/(nx*ny)
!print *,'jt,jz,Cs_4,Cs_2,Cs',jt,jz,sum(Cs_opt2_4d(1:nx,1:ny,jz))/(nx*ny),&
!sum(Cs_opt2_2d(1:nx,1:ny,jz))/(nx*ny),sum(Cs_opt2(1:nx,1:ny,jz))/(nx*ny)
!print *,'jt,jz,Cs_4/Cs_2,Cs_2/Cs',jt,jz,sum(Cs_opt2_4d(1:nx,1:ny,jz))/&
!sum(Cs_opt2_2d(1:nx,1:ny,jz)),sum(Cs_opt2_2d(1:nx,1:ny,jz))/sum(Cs_opt2(1:nx,1:ny,jz))
...
!!$!DY For debugging
!!$!DY Start here
!!$print*, "stop in lagrange_Sdep,coord=",coord
!!$stop
!!$!DY End here
!!$!DY*********************
