...
!Parameters
  ...
  INTEGER,PARAMETER                          :: jt_start = 175
  INTEGER,PARAMETER                          :: jt_end = 275
  ...
  INTEGER                                     ::ip,jt,jx,jy,jz,jt_total,nzs,ipcon,reclen
  logical                                     :: exist
...
write(file,'(A,I6.6,A)')'pcon_',jt_total,'.out'
...
write(20)PCon(1:nx,1:ny,1:nz_tot,1:npcon)!,avg_rhs1(1:nx,1:ny,1:nz_tot,1:npcon),avg_rhs2(1:nx,1:ny,1:nz_tot,1:npcon),&
!          avg_dissip(1:nx,1:ny,1:nz_tot)
close(20)

!write(file,'(A,I3.3,A,I3.3,A)')'avweps_',jt_start,'_',jt_end,'.out'
!open(unit=20,File=file,form='unformatted',access='direct',recl=reclen)
!write(20,rec=1) w(1:nx,1:ny,1:nz_tot)
!write(20,rec=2) dissip(1:nx,1:ny,1:nz_tot)
!close(20)

!write(file,'(A,I6.6,A)')'uvweps_',jt_total,'.out'
!open(unit=20,File=file,form='unformatted',access='direct',recl=reclen)
!write(20,rec=1) u(1:nx,1:ny,1:nz_tot)
!write(20,rec=2) v(1:nx,1:ny,1:nz_tot)
!write(20,rec=3) w(1:nx,1:ny,1:nz_tot)
!write(20,rec=4)dissip(1:nx,1:ny,1:nz_tot)
!close(20)
