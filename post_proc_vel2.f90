...
!Parameters
  ...
  REAL(rprec),dimension(:,:,:,:),allocatable ::Pcon,breakage_freq,Re,tot_vol,pcon0,tot_area
...
real(rprec)                       ::               dx=L_x/nx,dy=L_y/ny,dz=z_i/(nz_tot)
  real(rprec),dimension(:),allocatable        ::weight_x,weight_y,weight_z,d50,d32
  real(rprec),dimension(:,:),allocatable            ::M_tot,M_y,M_z,y_cm,z_cm,tot
real(rprec),parameter::pi=3.1415926535897932384626433_rprec
  dir ='output_data/'
   allocate(Pcon(nx,ny,1:nz_tot,npcon),tot_vol(nx,ny,1:nz_tot,npcon),tot_area(nx,ny,1:nz_tot,npcon))
...
allocate(d50(310-xps+1),d32(310-xps+1),tot(310-jx+1,npcon-1))
...
!  
     ...
     tot_area(:,:,:,ip) = pi*pcon(:,:,:,ip)*diameter(ip)**2_rprec
!!     write(*,*) maxval(tot_vol(:,:,:,ip))
     enddo

505 format(18e15.7e2)
...
!open(unit=22,file='dsd_init.dat',status='unknown')
!write(22,505)(pcon(59,yps,271,ip)/d_d(ip),ip=1,npcon)
!              
!close(22)
...
do jx=xps,310
  ...
  d32(jx-xps+1) =6*sum(tot_vol(jx,jy,jz,1:npcon))/&
                 sum(tot_area(jx,jy,jz,1:npcon))

enddo


write(file,'(A,A,I3.3,A)') TRIM(PATH) , "d32_incl", int((384-jz-1)*100/385),".dat"


open(unit=24,FILE=file,status='unknown')
write(24,*)'variables=x,d32'
write(24,*) 'zone t="',1,'"i=',310-xps+1,'f=point'
   
do jx = xps,310
  write(24,505) (jx-1)*l_x/nx,d32(jx-xps+1)
enddo
...
!   jz = 173
!   counter=1
!   print *, pcon(jx_start,yps,jz_start,1)
!   print *, d_d
