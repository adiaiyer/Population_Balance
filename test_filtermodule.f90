...
integer,parameter::filter_size=1._rprec
...
call rfftwnd_f77_one_real_to_complex(forw,f)
! the normalization is "in" G_test
f = G_test*f  ! Nyquist taken care of with G_test
call rfftwnd_f77_one_complex_to_real(back,f)
...
! alpha is ratio of test filter to grid filter widths
   kc2 = (pi/(delta))**2
   where (real(k2) >= kc2) G_test = 0._rprec
