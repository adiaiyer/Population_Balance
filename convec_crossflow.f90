...
!DY       +0.5_rprec*u3_big(:,:,2)*(-vort1_big(:,:,2)))
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    cc_big(:,:,1)=const*((u1_big(:,:,1)+ust(1)+ucross)*(vort3_big(:,:,1))&
...
!DY        +u3_big(:,:,jz)*(-vort1_big(:,:,jz))))
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    cc_big(:,:,jz)=const*((u1_big(:,:,jz)+ust(jz)+ucross)*(vort3_big(:,:,jz))&
...
!DY         )
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    cc_big(:,:,jz)=const*0.5_rprec*(&
        (u1_big(:,:,jz)+u1_big(:,:,jz-1)+ust(jz)+ust(jz-1)+2._rprec*ucross)*(-vort2_big(:,:,jz))&
