...
real(rprec)                       ::               dx=L_x/nx,dy=L_y/ny,dz=z_i/(nz_tot)
  ...
  real(rprec),dimension(:,:),allocatable::M_tot,M_y,M_z,y_cm,z_cm,d32,rhs_source
real(rprec),parameter::pi=3.1415926535897932384626433_rprec
 integer,parameter :: n=6
  ...
  allocate(u0(ld,ny,1:nz),v0(ld,ny,1:nz),w0(ld,ny,1:nz),pcon0(ld,ny,1:nz,npcon),dissip0(ld,ny,1:nz),& 
   ...
   allocate(d32(n,jt_end-jt_start+1),tot(npcon-1),rhs_source(n,npcon))
...
!!!!!!!!!!!!!!! D32 PDF  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
    d32(i,jt-jt_start+1) =6*sum(tot_vol(jx,jy,jz,1:npcon))/&
                            tot_area(jx,jy,jz)
    d32_dist = 6*(0.5_rprec*sum(tot_vol(jx,jy,jz,1:14)) + 0.5_rprec*sum(tot_vol(jx,jy,jz,1:13)))+/&
                                    tot_area(jx,jy,jz)
  enddo 
  ...
  write(100,505) (jt)*base*dt_dim, (d32_dist(i,jt-jt_start+1),i=i,n)
...
deallocate(u0,v0,w0,dissip0,pcon0,tot_vol,tot,d32,pcon,dissip,rhs1,tot_Area,Re0,breakage_freq0)
