...
call oil_properties
    ! Check mass balance
if (coord.eq.0 .and. jt.eq. Pcon_init) then
...
!  e = 10**(0.3637_rprec*exp(-33.29_rprec*log10(Oh+1)) + 0.5727_rprec*exp(-0.4443*log10(Oh+1)))
  a = 2.392_rprec*exp(-26.76_rprec*Oh)+2.877_rprec*exp(-0.1244_rprec*Oh)
  b = 0.45_rprec
  c = -10**((0.5446_rprec*Oh**(-0.3776_rprec)/(1+12.67_rprec*Oh**(1.462_rprec))))
  d = -(4.172_rprec*Oh**(-0.5492_rprec))/(1+0.5079_rprec*Oh**(-0.4879_rprec))
  e = 10**(0.4113_rprec*exp(-55.94_rprec*log10(Oh+1)) + 0.5125_rprec*exp(-0.7182*log10(Oh+1)))
  ...
  do ip=1,npcon
    dissip_scaled = dissip*(u_star**3._rprec)/z_i ! Scale dissipation with u_star
...
!    endif
 ...
 !   breakage_freq(:,:,:,ipcon)=k_b*pi/4*0.812*tau_c(:,:,:,ipcon)*Re(:,:,:,ipcon)**2*fit(:,:,:,ipcon)
    breakage_freq(:,:,:,ip) = k_b*(2.1_rprec)**(0.5_rprec)*pi/4._rprec*0.84_rprec*tau_c(:,:,:,ip)*fit(:,:,:,ip)
enddo
...
!              do jy=jl,jr
!                 do jx=il,ir
! Width of scalar plume is twice that of velocity
              Gaus=0._rprec
              do jy=1,ny
                 do jx=1,nx
                    Gaus=Gaus+exp(-(((jx-xps)*dx)**2+((jy-yps)*dy)**2)**(N_GAU/2)/(2._rprec*(2*b0_plume)**N_GAU)) &
                         /(2._rprec*pi*(2*b0_plume/dx)**2)
                 enddo
              enddo
              ...
              do jy=1,ny
                 do jx=1,nx
                    RHS(jx,jy,zps_local,ipcon)=RHS(jx,jy,zps_local,ipcon)+(0.7_rprec)*Q_src(ipcon) & 
                         *exp(-(((jx-xps)*dx)**2+((jy-yps)*dy)**2)**(N_GAU/2)/(2._rprec*(2*b0_plume)**N_GAU)) &
                         /(2._rprec*pi*(2*b0_plume/dx)**2)/Gaus
                    RHS(jx,jy,zps_local+1,ipcon)=RHS(jx,jy,zps_local+1,ipcon)+(0.3_rprec)*Q_src(ipcon) & 
                         *exp(-(((jx-xps)*dx)**2+((jy-yps)*dy)**2)**(N_GAU/2)/(2._rprec*(2*b0_plume)**N_GAU)) &
                         /(2._rprec*pi*(2*b0_plume/dx)**2)/Gaus
                 enddo
              enddo
...
!countloop:do while(counter_pbe.le.npcon)
...
!
    ...
    do jpcon=ipcon+1,npcon
!       write(*,*) minval(Rhs_pbe_in),maxval(Rhs_pbe_in)
       do jz=1,nz-1
       do jy=1,ny
       do jx=1,nx
      temp_conc(jx,jy,jz)=breakage_freq(jx,jy,jz,jpcon)*beta(ipcon,jpcon)*scalar(jx,jy,jz,jpcon)
      counter_pbe=2
 !     check_neg(:,:,:,jpcon) = -temp_conc+scalar(:,:,:,jpcon)
   !  do jx=1,nx
   !    do jy=1,ny
   !      do jz=1,nz-1 
   !        if (temp_conc(jx,jy,jz) .gt. scalar(jx,jy,jz,jpcon)) then
   !            write (111,*) "jt jx,jy,jz,temp_conc
   !            Scalar",jt,jx,jy,jz,temp_conc,scalar(jx,jy,jz,jpcon)
   !        endif
   !      enddo
   !    enddo
   !  enddo
      Rhs_pbe_in(jx,jy,jz,ipcon) = Rhs_pbe_in(jx,jy,jz,ipcon) +temp_conc(jx,jy,jz)
      Rhs_pbe_in(jx,jy,jz,jpcon) = Rhs_pbe_in(jx,jy,jz,jpcon)-temp_conc(jx,jy,jz)
      d_0 = (dcube(jpcon) - dcube(ipcon))**(1._rprec/3._rprec)
      if(d_0 .gt. diameter(1)) then
countloop:do while(counter_pbe .le. npcon)
          if(d_0>diameter(counter_pbe)) then
            ...
            b1=abs(diameter(counter_pbe-1)-d_0)
            ab_sum=a1+b1
            Rhs_pbe_in(jx,jy,jz,counter_pbe-1)=Rhs_pbe_in(jx,jy,jz,counter_pbe-1)+temp_conc(jx,jy,jz)*(a1/ab_sum)*(d_0**3/dcube(counter_pbe-1))
            Rhs_pbe_in(jx,jy,jz,counter_pbe)=Rhs_pbe(jx,jy,jz,counter_pbe)+temp_conc(jx,jy,jz)*(b1/ab_sum)*(d_0**3/dcube(counter_pbe))
            exit countloop
          endif
        enddo countloop
      else
        Rhs_pbe_in(jx,jy,jz,1)=Rhs_pbe_in(jx,jy,jz,1)+temp_conc(jx,jy,jz)*(d_0**3/diameter(1)**3)
      endif
     enddo
     enddo
     enddo
...
!beta is the probability that drop j breaks up into drop of size i
  ...
  real(kind=rprec),dimension(npcon,npcon),intent(out):: beta
