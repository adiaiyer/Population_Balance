...
!AA Added by Aditya Aiyer for Population Balance Equations
  ...
  real(kind=rprec)                                 ::a1,b1,d_0,ab_sum,fit_start,fit_end,int_start,int_end,&
                                                     ans
  integer                                          ::counter_pbe,m,ip,num_pts=1000
  real(kind=rprec),dimension(npcon)                :: diameter,dcube,dsquare
  real(kind=rprec),dimension(ld,ny,$lbz:nz)        :: ones
  real(kind=rprec),dimension(1000)                 :: r,func
!  integer                           :: ipcon
!AA End Here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
call cpu_time(fit_start)
  a = 2.374_rprec*exp(-19.88_rprec*Oh)+2.788_rprec*exp(-0.07416_rprec*Oh)
  b = 0.45_rprec
  c = -10**((1.41_rprec*Oh**(-0.245_rprec)/(1+5.178_rprec*Oh**(-0.83_rprec))))
  d = (-5.313_rprec*Oh**(-0.4541_rprec))/(1+0.4981_rprec*Oh**(-0.422_rprec))
  e = 10**(0.415_rprec*exp(-41.09_rprec*log10(Oh+1)) + 0.5088_rprec*exp(-0.4604*log10(Oh+1)))
...
!
!do ip=1,npcon
!    r = (/(1.e-6 +(i-1)*(diameter(ip)-1.e-6)/(num_pts-1.),i=1,1000,1)/)
!  do jz=1,nz
!  do jy = 1,ny
!  do jx = 1,nx
!    func =Re(jx,jy,jz,ip)*r**(-11./3.)
!    ans = integrate(r,func,1e-6_rprec,1._rprec)
!
!  enddo
!  enddo
!  enddo
...
!open(70,file="intvsfitcputime.dat",status='unknown',position='append')
!write(70,*) int_end-int_start , fit_end - fit_start
!close(70)
