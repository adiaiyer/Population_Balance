...
real(kind=rprec),dimension($lbz:nz) :: ust, vst
real (rprec) :: z
!DY End here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
if(OCEAN_FLAG .and. STOKES_FLAG) then
   do jz=$lbz,nz
      ...
      $endif
      ust(jz)=U_stokes*exp(-2._rprec*wavenm_w*z)*cos(agl_Ust)
      vst(jz)=U_stokes*exp(-2._rprec*wavenm_w*z)*sin(agl_Ust)
   enddo
endif
!+++++++++++++++
!DY End here
...
!DY            +(w_m(i,j,k)*dsdz_m(i,j,k)+w_m(i,j,k+1)*dsdz_m(i,j,k+1))/2._rprec
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            !-BC RHS_m(i,j,k)=(u_m(i,j,k)+ust(k))*dsdx_m(i,j,k)+v_m(i,j,k)*dsdy_m(i,j,k)&
            !-BC     +(w_m(i,j,k)*dsdz_m(i,j,k)+w_m(i,j,k+1)*dsdz_m(i,j,k+1))/2._rprec
          !-BC added by Bicheng Chen for direction of Stokes drift
          RHS_m(i,j,k)=(u_m(i,j,k)+ust(k))*dsdx_m(i,j,k)+&
            (v_m(i,j,k)+vst(k))*dsdy_m(i,j,k)&
            +(w_m(i,j,k)*dsdz_m(i,j,k)&
            +w_m(i,j,k+1)*dsdz_m(i,j,k+1))/2._rprec
          !-BC END added by Bicheng Chen for direction of Stokes drift
         else
...
!DY!        RHS_m(i,j,Nz)=u_m(i,j,Nz)*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            !-BC RHS_m(i,j,1)=(u_m(i,j,1)+ust(1))*dsdx_m(i,j,1)+v_m(i,j,1)*dsdy_m(i,j,1)&
            !-BC     +(0.5_rprec*w_m(i,j,2))*dsdz_m(i,j,2)
            !-BC added by Bicheng Chen for direction of Stokes drift
            RHS_m(i,j,1)=(u_m(i,j,1)+ust(1))*dsdx_m(i,j,1)&
              +(v_m(i,j,1)+vst(1))*dsdy_m(i,j,1)&
...
!DY        RHS_m(i,j,Nz)=u_m(i,j,Nz)*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            !-BC RHS_m(i,j,Nz)=(u_m(i,j,Nz)+ust(Nz))*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
          !-BC added by Bicheng Chen for direction of Stokes drift
            RHS_m(i,j,Nz)=(u_m(i,j,Nz)+ust(Nz))*dsdx_m(i,j,Nz)&
            +(v_m(i,j,Nz)+vst(Nz))*dsdy_m(i,j,Nz)
          !-BC END added by Bicheng Chen for direction of Stokes drift
         else
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!DY Added by Di Yang for ocean LES with Stokes drift
!BC Added by Bicheng Chen for Stokes drift angle
real(kind=rprec),dimension($lbz:nz) :: ust, vst
real (rprec) :: z
!END BC Added by Bicheng Chen for Stokes drift angle
!DY End here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++

!+++
!BC Added By Bicheng Chen for determing whether a bondary is a inlet or outlet
real(kind=rprec),dimension(ny,$lbz:nz) :: ust_x=0, one_x=1._rprec
real(kind=rprec),dimension(nx,$lbz:nz) :: vst_y=0, one_y=1._rprec
...
!          term1=((2._rprec*z_os(i,j))/dz)**(Sc_boun*settling_vel/(vonk*ustar_local(i,j)))
  ...
  ELSE
    ...
    ! scalar_x(1,1:ny,1:nz-1)=scalar(1,1:ny,1:nz-1,ipcon)
    ...
    ! ghost_x0(1:ny,1:nz-1)=scalar(1,1:ny,1:nz-1,ipcon)
    ...
    if(OCEAN_FLAG .and. STOKES_FLAG) then
      ! x0 bdy
      ghost_x0(1:ny,1:nz-1) = scalar(1,1:ny,1:nz-1,ipcon)*(1-sign(one_x(1:ny,1:nz-1),u_int(1,1:ny,1:nz-1)+ust_x(1:ny,1:nz-1)))/2
      scalar_x(1,1:ny,1:nz-1) = ghost_x0(1:ny,1:nz-1)
      ! xLx bdy
      ghost_xLx(1:ny,1:nz-1) = scalar(nx,1:ny,1:nz-1,ipcon)*(1-sign(one_x(1:ny,1:nz-1),-(u_int(1,1:ny,1:nz-1)+ust_x(1:ny,1:nz-1))))/2
      scalar_x(nx+1,1:ny,1:nz-1) = ghost_xLx(1:ny,1:nz-1)
    else
      ! x0 bdy
      ghost_x0(1:ny,1:nz-1) = scalar(1,1:ny,1:nz-1,ipcon)*(1-sign(one_x(1:ny,1:nz-1),u_int(1,1:ny,1:nz-1)))/2
      scalar_x(1,1:ny,1:nz-1) = ghost_x0(1:ny,1:nz-1)
      ! xLx bdy
      ghost_xLx(1:ny,1:nz-1) = scalar(nx,1:ny,1:nz-1,ipcon)*(1-sign(one_x(1:ny,1:nz-1),-(u_int(1,1:ny,1:nz-1))))/2
      scalar_x(nx+1,1:ny,1:nz-1) = ghost_xLx(1:ny,1:nz-1)
    endif
...
!    ghost_yLy(1:nx,1:nz-1)=scalar(1:nx,ny,1:nz-1)
    ...
    !BC modified by Bicheng Chen, not an inlet
    ! scalar_y(1:nx,ny+1,1:nz-1)=scalar(1:nx,ny,1:nz-1,ipcon)
    ...
    ! ghost_yLy(1:nx,1:nz-1)=scalar(1:nx,ny,1:nz-1,ipcon)
    !END BC modified by Bicheng Chen, not an inlet
    if(OCEAN_FLAG .and. STOKES_FLAG) then
      ! y0 bdy
      ghost_y0(1:nx,1:nz-1) = scalar(1:nx,1,1:nz-1,ipcon)*(1-sign(one_y(1:nx,1:nz-1),v_int(1:nx,1,1:nz-1)+vst_y(1:nx,1:nz-1)))/2
      scalar_y(1:nx,1,1:nz-1) = ghost_y0(1:nx,1:nz-1)
      ! yLy bdy
      ghost_yLy(1:nx,1:nz-1) = scalar(1:nx,ny,1:nz-1,ipcon)*(1-sign(one_y(1:nx,1:nz-1),-(v_int(1:nx,1,1:nz-1)+vst_y(1:nx,1:nz-1))))/2
      scalar_y(1:nx,ny+1,1:nz-1) = ghost_yLy(1:nx,1:nz-1)
    else
      ghost_y0(1:nx,1:nz-1) = scalar(1:nx,1,1:nz-1,ipcon)*(1-sign(one_y(1:nx,1:nz-1),v_int(1:nx,1,1:nz-1)))/2
      scalar_y(1:nx,1,1:nz-1) = ghost_y0(1:nx,1:nz-1)
      ! yLy bdy
      ghost_yLy(1:nx,1:nz-1) = scalar(1:nx,ny,1:nz-1,ipcon)*(1-sign(one_y(1:nx,1:nz-1),-(v_int(1:nx,1,1:nz-1))))/2
      scalar_y(1:nx,ny+1,1:nz-1) = ghost_yLy(1:nx,1:nz-1)
    endif
  ...
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    DO k=1,nz-1
      DO i=1,nx
        ...
        ! Choose the appropriate upwind direction
        IF (v_int(i,ny,k)+vst(k) >= 0._rprec) THEN  
          scalar_y(i,ny,k)=(3._rprec*scalar(i,ny,k,ipcon)&
          +6._rprec*scalar(i,ny-1,k,ipcon)-scalar(i,ny-2,k,ipcon))/8._rprec
        ELSE    
          scalar_y(i,ny,k)=(3._rprec*scalar(i,ny-1,k,ipcon)&
          +6._rprec*scalar(i,ny,k,ipcon)-ghost_yLy(i,k))/8._rprec
        END IF
      ...
      if(OCEAN_FLAG .and. STOKES_FLAG) then
        DO k=1,nz-1
          DO i=1,nx
            ...
            ! Last node (j=ny+1)
            ! Choose the appropriate upwind direction
            IF (v_int(i,ny+1,k)+vst(k) >= 0._rprec) THEN  
...
!BC Modified by Bicheng Chen for multiple sources
     ncpu_source = int(zps(ipcon)/(nz-1))
     zps_local = zps(ipcon)-ncpu_source*(nz-1)+1

     if ((.not. USE_MPI) .or. (USE_MPI .and. coord == ncpu_source)) then

        IF (single_point) THEN

           RHS(xps(ipcon),yps(ipcon),zps_local,ipcon)= &
            RHS(xps(ipcon),yps(ipcon),zps_local,ipcon) &
            +(0.75_rprec)*Q_src(ipcon)
           RHS(xps(ipcon),yps(ipcon),zps_local+1,ipcon)= &
            RHS(xps(ipcon),yps(ipcon),zps_local+1,ipcon) &
            +(0.25_rprec)*Q_src(ipcon)
          !BC END Modified by Bicheng Chen for multiple sources
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    !BC debug ust(k)->ust(1:nz-1))
     !flux_out=(SUM((u_int(nx+1,1:ny,1:nz-1)+ust(k))*scalar_x(nx+1,1:ny,1:nz-1))*dt*dy*dz)
     do j = 1,ny
        flux_out=flux_out+&
        (SUM((u_int(nx+1,j,1:nz-1)+ust(1:nz-1))*scalar_x(nx+1,j,1:nz-1))*dt*dy*dz)
     end do
  else
  ...
  if(OCEAN_FLAG .and. STOKES_FLAG) then
    do i = 1,nx
      flux_out=flux_out+&
        (SUM((v_int(i,1,1:nz-1)+vst(1:nz-1))*&
        scalar_x(i,1,1:nz-1))*dt*dx*dz)
    end do
  else
    flux_out=flux_out+&
      (SUM(v_int(1:nx,1,1:nz-1)*scalar_x(1:nx,1,1:nz-1))*dt*dx*dz)
  end if
  !BC END added by Bicheng Chen for direction of Stokes drift
!+++++++++++++++
!DY End here
